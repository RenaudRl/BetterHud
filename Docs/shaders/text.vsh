...
void main() {
    ...
    if (pos.y >= ui.y && ProjMat[3].x == -1) {
        ...
        if (ProjMat[3].x == -1 && range(pos.y, ui.y - 60, ui.y - 20) && range(pos.x, ui.x / 2 - 60, ui.x / 2 + 60) && (range(color, exp / 256, exp / 254) || color == vec3(0))) {
            vertexColor = vec4(0);
        }
    }
    ...
    // ScreenEffects integration logic
    float screenEffectId = floor((pos.y + 1000.0) / 2000.0);
    if (screenEffectId >= 4.0 && screenEffectId <= 5.0) {
        ...
        // Sliding Titles integration logic
        if (Position.z == 2400.12 || Position.z == 2400.06 || Position.z == 2400.0) {
            float guiScale = (round(ScreenSize.x * ProjMat[0][0] / 2.0));
            float t = vertexColor.a;
            float posAlpha = t * t * (3.0 - 2.0 * t);
            float offset = TITLE_OFFSET * TITLE_SCALE;
            float finalPos = 1.1 - posAlpha * offset * guiScale;

            mat4 TransformMat = mat4(mat3(TITLE_SCALE));
            TransformMat[3].y = finalPos;

#ifndef FADE
            vertexColor.a = 1.0;
#endif
            gl_Position = TransformMat * ProjMat * ModelViewMat * vec4(pos, 1.0);
        } else {
