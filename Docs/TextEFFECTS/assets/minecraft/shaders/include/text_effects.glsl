...
void apply_layer_glowing(vec3 color, float intensity) {
    if(textData.isShadow) return;
    vec3 d = textSdf();
    float glow = (1.0 - d.z) * (1.0 - d.z) * intensity;
    textData.backColor = mix(textData.backColor, vec4(color, 1.0), glow);
}

void apply_layer_glowing() {
    apply_layer_glowing(vec3(1.0), 1.0);
}

void apply_layer_background(vec4 color, float padding) {
    if(textData.isShadow) return;
    // Simple box background based on localPosition
    // padding is in pixels (approx)
    float pad = padding / 8.0; // Normalized approximate padding
    if (textData.localPosition.x >= -pad && textData.localPosition.x <= 1.0 + pad &&
        textData.localPosition.y >= -pad && textData.localPosition.y <= 1.0 + pad) {
        textData.backColor = mix(textData.backColor, color, color.a);
    }
}

void apply_layer_background(vec3 color) {
    apply_layer_background(vec4(color, 1.0), 0.0);
}

void apply_layer_rainbow_outline(float speed) {
    if(textData.isShadow) return;
    vec3 rainbow = hsvToRgb(vec3(textData.characterPosition.x * 0.01 - GameTime * 12800.0 * speed, 0.8, 1.0));
    apply_outline(rainbow);
}

void apply_layer_glitch(float intensity) {
    if(textData.isShadow) return;
    float t = GameTime * 48000.0;
    if (random(vec2(floor(t), 0.0)) > 0.9) {
        float shift = (random(vec2(floor(t), 1.0)) - 0.5) * intensity / 256.0;
        textData.uv.x += shift;
    }
}
